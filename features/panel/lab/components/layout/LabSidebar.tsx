...
import { labSidebarItems } from "@/features/panel/lab/constants";
...
export default function LabSidebar({ collapsed = false, onToggleCollapse }: Props) {
    ...
    >
      ...
      >
        ...
        <div className={cn("leading-none", collapsed && "hidden")}>
          ...
          <p className="mt-1 text-[10px] text-slate-500 dark:text-[#64748B]">Laboratory Panel</p>
        </div>
      </div>

      <div className="flex-1 overflow-y-auto px-[8px] py-3">
        {labSidebarItems.map((group, index) => (
            ...
            <div className="space-y-[1px]">
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <div className="absolute left-0 top-1/2 h-[18px] w-[3px] -translate-y-1/2 rounded-r-[3px] bg-[#3B82F6]" />
                    )}
