    ...
    <div className="px-6 py-5">
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[7fr_5fr]">
        <div className="space-y-3">
          {reviews.map((review) => (
            <div key={`${review.user}-${review.date}`} className={`rounded-[12px] border p-4 ${review.pending ? 'border-[rgba(245,158,11,.2)] bg-[rgba(245,158,11,.03)]' : 'border-white/[0.07] bg-[#161f30]'}`}>
              <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="font-medium text-[#f1f5f9]">{review.user}</div>
              ...
              <p className="mb-3 text-[12.5px] leading-6 text-[#94a3b8]">{review.text}</p>
              <p className="mb-2 text-[11px] text-[#475569]">For: <span className="text-[#e2e8f0]">{review.doctor}</span></p>

              {review.reply ? (
                <div className="mb-2 rounded-lg border border-[rgba(59,130,246,.1)] bg-[rgba(59,130,246,.06)] p-3 text-[11.5px] text-[#cbd5e1]">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button className="rounded border border-white/[0.08] bg-white/[0.04] px-2.5 py-1 text-[11px] text-[#94a3b8] hover:bg-white/[0.08]">Mark Resolved</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="rounded-[14px] border border-white/[0.07] bg-[#161f30] p-5">
          <h3 className="text-[13px] font-semibold text-[#f1f5f9]">Rating Breakdown</h3>
          <p className="mt-1 text-[11px] text-[#94a3b8]">Based on 1,240 verified reviews</p>
          <div className="mt-4 space-y-3">
            {[['5 ★', 74], ['4 ★', 20], ['3 ★', 4], ['2 ★', 1], ['1 ★', 1]].map(([label, val]) => (
              <div key={label}>
                <div className="mb-1 flex items-center justify-between text-[11px] text-[#94a3b8]"><span>{label}</span><span>{val}%</span></div>
                <div className="h-[6px] rounded bg-white/[0.06]">
