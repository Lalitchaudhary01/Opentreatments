  ...
  return (
    <div className="px-6 py-5">
      <div className="grid gap-4 xl:grid-cols-[200px_1fr]">
        <aside className="sticky top-0 h-fit overflow-hidden rounded-[14px] border border-white/[0.07] bg-[#161f30]">
          {nav.map(([label, Icon], idx) => (
            <button
              key={label}
              className={`flex w-full items-center gap-2 border-b border-white/[0.07] px-4 py-3 text-left text-[12.5px] transition last:border-b-0 ${
                idx === 0
        ...
        <section className="overflow-hidden rounded-[14px] border border-white/[0.07] bg-[#161f30]">
          <div className="border-b border-white/[0.07] px-5 py-4">
            <h2 className="text-[13px] font-semibold text-[#f1f5f9]">General Settings</h2>
            <p className="mt-0.5 text-[11px] text-[#94a3b8]">Control hospital preferences and operational defaults</p>
          </div>

          <div>
            {rows.map(([title, sub, value], idx) => (
              <div key={title} className={`flex flex-wrap items-center justify-between gap-3 px-5 py-4 ${idx !== rows.length - 1 ? 'border-b border-white/[0.07]' : ''}`}>
                <div>
                  <h3 className="text-[13px] font-medium text-[#f1f5f9]">{title}</h3>
                ...
                {value === "enabled" ? (
                  ...
                  <input
                    defaultValue={value}
                    className="h-8 min-w-[220px] rounded-lg border border-white/[0.08] bg-white/[0.04] px-3 text-[12px] text-[#e2e8f0] outline-none focus:border-[#3b82f6]/40"
                  />
                )}
              </div>
            ))}
          ...
          <div className="flex items-center justify-end gap-2 border-t border-white/[0.07] px-5 py-3">
            <button className="rounded border border-white/[0.08] bg-white/[0.04] px-3 py-1.5 text-[11px] text-[#94a3b8] hover:bg-white/[0.08]">Cancel</button>
