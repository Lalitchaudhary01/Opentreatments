...
function categoryPill(category: string) {
  ...
  if (category === "Lab Test") return "bg-[rgba(148,163,184,.12)] text-[#94a3b8]";
    ...
    <div className="px-6 py-5">
          ...
          return (
            <div key={s.label} className="rounded-[13px] border border-white/[0.07] bg-[#161f30] p-5">
              <div className="mb-3 flex items-start justify-between">
                <div className={`flex h-[34px] w-[34px] items-center justify-center rounded-[9px] bg-white/[0.06] ${s.color}`}>
      ...
      <section className="mt-4 overflow-hidden rounded-[14px] border border-white/[0.07] bg-[#161f30]">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.07] px-5 py-4">
          <div>
            <h2 className="text-[13px] font-semibold text-[#f1f5f9]">Service Catalog</h2>
            <p className="mt-0.5 text-[11px] text-[#94a3b8]">Manage consultations, procedures and packages</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row[0]} className={`border-t border-white/[0.07] text-[12.5px] ${row[5] === 'Archived' ? 'opacity-60' : ''}`}>
                  <td className="px-5 py-3 font-medium text-[#e2e8f0]">{row[0]}</td>
                  ...
                  <td className="px-5 py-3 text-[#94a3b8]">{row[2]}</td>
                  <td className="px-5 py-3 text-[#94a3b8]">{row[3]}</td>
                  <td className="px-5 py-3 font-semibold text-[#f1f5f9]">{row[4]}</td>
                  ...
                  <td className="px-5 py-3 text-right">
                    <div className="inline-flex items-center gap-1.5">
                      <button className="rounded border border-white/[0.08] bg-white/[0.04] px-2.5 py-1 text-[11px] text-[#94a3b8] hover:bg-white/[0.08]">Edit</button>
                      <button className="rounded border border-white/[0.08] bg-white/[0.04] px-2.5 py-1 text-[11px] text-[#94a3b8] hover:bg-white/[0.08]">{row[5] === 'Archived' ? 'Restore' : 'Archive'}</button>
                    </div>
                  </td>
                </tr>
              ))}
